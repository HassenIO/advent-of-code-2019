...
func TestFuelCalculator(t *testing.T) {
	if fuel := FuelCalculator(14); fuel != 2 {
		t.Errorf("FuelCalculator(14) = %d; want 2", fuel)
	}

	if fuel := FuelCalculator(2); fuel != 0 {
		t.Errorf("FuelCalculator(2) = %d; want 0", fuel)
	}

	if fuel := FuelCalculator(1969); fuel != 966 {
		t.Errorf("FuelCalculator(1969) = %d; want 966", fuel)
	}

	if fuel := FuelCalculator(100756); fuel != 50346 {
		t.Errorf("FuelCalculator(100756) = %d; want 50346", fuel)
	}
}
